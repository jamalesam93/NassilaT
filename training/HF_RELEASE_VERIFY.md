# HF release verification — Sanad S15 / S14
...
|---------|-----------|------------|------|----------|-----------------|-----------------|
| `QinEmPeRoR93/nassila-sanad-4b` | `nassila-sanad-4b-q6_k.gguf` | **S15** *(v1.15)* | Default-tier (contrastive v2) | **94.48%** | *pending local verify* | **4.87%** |
| `QinEmPeRoR93/nassila-sanad-12b` | `nassila-sanad-12b-q6_k.gguf` | **S14** *(legacy v1.14)* | Tier 2 | **90.43%** | **100%** | **2.86%** |
| `QinEmPeRoR93/nassila-sanad-e4b` | `nassila-sanad-e4b-q6_k.gguf` | **S12** *(legacy v1.12)* | **RETIRED** | 89.27% | 92.98% | 3.81% |

**Do not publish** v1.13 / S13 12B. v1.12 12B remains higher-combined fallback/reference only. S15 single-run provenance — quote figure requires laptop re-verify before treating as measured.
...
| Artifact | Path |
|----------|------|
| 4B model card | [`hf_readmes/nassila-sanad-4b/README.md`](./hf_readmes/nassila-sanad-4b/README.md) |
| 12B model card | [`MODEL_CARD_sanad_12b.md`](./MODEL_CARD_sanad_12b.md) |
| E4B (retired) card | [`MODEL_CARD_sanad_e4b.md`](./MODEL_CARD_sanad_e4b.md) |
...
| Vast reports | `reports/ab_12b_q6_k_v114/`, contrastive v2 reports |
...
- [x] 4B (S15) + 12B (S14) metrics in model cards / HF READMEs — provenance noted (S15 single-run contrastive v2; quote pending)
- [x] E4B (S12) marked **retired** in card + HF README
- [x] HF README `base_model` correct (`Qwen/Qwen3.5-4B` / `google/gemma-4-12B-it` / `google/gemma-4-E4B-it`)
- [x] GGUF filenames on HF match ship table
- [x] v1.13 GGUF **not** uploaded under any tag
- [x] Nassila `OUROBOROS_CONTEXT.md` / `PRODUCT.md` reference S15 4B + S14 12B
- [ ] **S15 laptop smoke + quote eval** — pending (contrastive v2 is single-run; backfill quote)
