...
**Recommended** — download this GGUF, load it in [LM Studio](https://lmstudio.ai/), and start the Local Server at `http://localhost:1234`.

In Nassila: **Settings → Passage grounding** → runner **LM Studio** → model `nassila-sanad-4b` (or the id LM Studio shows).
...
```bash
ollama pull huggingface.co/QinEmPeRoR93/nassila-sanad-4b:Q6_K
```

In Nassila: runner **Ollama** → base URL `http://localhost:11434` → model name from `ollama list` (often `nassila-sanad-4b:Q6_K`).

<details>
<summary>Modelfile fallback (private repo or pull tag not indexed)</summary>

```
FROM https://huggingface.co/QinEmPeRoR93/nassila-sanad-4b/resolve/main/nassila-sanad-4b-q6_k.gguf
PARAMETER num_ctx 4096
```

```bash
ollama create nassila-sanad-4b -f Modelfile
```
...
Serve the GGUF with any OpenAI-compatible server (`ctx-size` 4096). Point Nassila at your base URL and exposed model id.

```bash
llama-server -m nassila-sanad-4b-q6_k.gguf \
