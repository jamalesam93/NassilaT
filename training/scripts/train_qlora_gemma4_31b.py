#!/usr/bin/env python3
"""
QLoRA fine-tuning for Nassila L3 grounding on Gemma 4 31B (premium tier).

Same recipe as 12B (2 ep, 1e-4, LoRA r=16) on shared v1.12 data.
Requires ~48GB+ VRAM for QLoRA (Vast A100 80GB recommended).

Usage:
  python scripts/train_qlora_gemma4_31b.py --phase 12 \\
...
PHASE_CONFIG = {
    "12": {
        "num_epochs": 2,
        "learning_rate": 1e-4,
        "output_name": "nassila-sanad-31b-v1.12",
    },
}


def main() -> int:
    parser = argparse.ArgumentParser(description="QLoRA for Nassila grounding on Gemma 4 31B")
    parser.add_argument(
        "--train-file",
        type=Path,
        default=TRAINING_DIR / "data" / "l3_grounding_train_v112.jsonl",
    )
    parser.add_argument("--phase", choices=tuple(PHASE_CONFIG.keys()), default="12")
    ...
    print(
        f"31B phase {args.phase}: base={BASE_MODEL}, epochs={phase['num_epochs']}, "
