...
2. **Body holdout** — contrastive v2 (308 rows) scored: S14 false-supported **4.22%**; S15 combined expect **94.48%**, false-supported **4.87%** (quote validity *pending local verify* — not measured on this harness).
...
|-------|--------|----------------------------|
| Quote validity (holdout) | ≥98% | *pending local verify* |
...
- [x] **S15 Trained & Merged** — `nassila-sanad-4b-q6_k.gguf` based on `Qwen/Qwen3.5-4B` (**94.48% combined expect, 4.87% false-supported**; quote pending local verify)
